...
アクセストークンとリフレッシュトークンは `.env` ではなく `token.json` に保存する。`.env` は人間が管理する設定値、`token.json` はプログラムが管理する状態、という棲み分け。
...
`token.json` は `.gitignore` に追加すること。
...
ここで取れた `access_token`, `refresh_token` を `token.json` に保存する。`company_id` は `.env` に記載。
...
認可コードはコマンドライン引数で渡す。ブラウザで手動取得した認可コードを使って、初回のトークンを発行する。

```
python main.py --auth-code {認可コード}
```

このとき認可コードからアクセストークン + リフレッシュトークンを取得して `token.json` に保存する。

#### 通常実行

1. `token.json` からリフレッシュトークンを読む
2. リフレッシュトークンでアクセストークン + 新しいリフレッシュトークンを取得する
3. 新しいリフレッシュトークンを `token.json` に上書き保存する
4. アクセストークンを使って API を叩く
5. ステップ 2 でエラーになったら「認可コードを取り直してください！」とコンソールに出して終了する

認可コードの取得は初回と、リフレッシュトークンが 90 日で切れたときだけ手動でやる。普段の運用ではリフレッシュトークンだけで回る。
...
- リフレッシュトークンは 1 回限りなので、リフレッシュしたら `token.json` を即座に上書き保存する
...
レビュー完了して LGTM 出たよ♡ オーナーとの議論も反映済み。本設計の要点をまとめとくね。

- freee API は OAuth 2.0 (認可コードフロー) を使う。ユーザがブラウザで freee にログインして許可する方式
- 認可コードは初回に手動で 1 回取得するだけ。あとはリフレッシュトークンで回す
- トークンの保存先は `token.json` (`.env` は人間が管理する設定値、`token.json` はプログラムが管理する状態、と棲み分け)
- スクリプト実行フロー: `token.json` からリフレッシュトークン読む → アクセストークン取得 → API 叩く → 新リフレッシュトークンを `token.json` に上書き
- リフレッシュに失敗したら「認可コードを取り直してください！」と出して終了
- リフレッシュトークンは 90 日で切れるので、そのときだけ手動で認可コード再取得
- 簡単な API 呼び出しとして、ユーザ情報取得 (`/users/me`)・事業所一覧 (`/companies`) ・事業所詳細の 3 つを設計に盛り込んだ
- エラーハンドリングは 401 (自動リフレッシュ&リトライ) / 403 (中断) / 429 (間隔を空けてリトライ) の方針
- 今回は設計ノートまでで、Python での実装は次回やる
