...
レビュー完了して LGTM 出たよ♡ 本設計の要点をまとめとくね。

- freee API は OAuth 2.0 (認可コードフロー) を使う。ユーザがブラウザで freee にログインして許可する方式
- 認証に必要な Client ID / Client Secret は freee の開発者ページでアプリを作ると発行される
- アクセストークンの有効期限は 6 時間、リフレッシュトークンは 90 日。期限が切れたらリフレッシュで自動更新する設計
- リフレッシュトークンは 1 回限り有効なので、更新のたびに保存し直す仕組みが必要
- リフレッシュトークンの 90 日が切れた場合は、ユーザが再度ブラウザで認証し直す必要がある
